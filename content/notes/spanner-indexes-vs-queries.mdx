...
The advantage of adding `STORING` over an index with just the other three keys is that it removes the
...
created which exceeds the task limit. What is the difference between using indexed keys and `STORING`
if we create one HIGH priority task then one MEDIUM priority task?
